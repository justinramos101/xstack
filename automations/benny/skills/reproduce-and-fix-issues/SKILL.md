...
- Use xstack's `principle-guard-the-context-window` for delegated analysis.
- Apply xstack's `principle-sequence-verifiable-units`, `principle-fix-root-causes`, and `principle-prove-it-works` through repro, fix, and verification.
...
2. Validate the normalized trigger with `node <copied Benny pack>/scripts/validate-trigger.mjs <configured-source-channel-id> < trigger.json`. Use its `SOURCE_CHANNEL_ID` and `SOURCE_THREAD_TS` output. A nonzero exit stops the run before any external write. The parser uses a nonempty `thread_ts`, otherwise `ts`.
...
Use xstack's `how` skill to trace the action through the repository. Use `why` for regression history and defensive code. Form competing cause hypotheses and identify evidence that would separate them.
...
- Invoke xstack's `tdd` skill when there is a cheap local test target, and write the failing test before the fix.
...
- Run the pull request text and all Slack updates through xstack's `unslop` skill.
