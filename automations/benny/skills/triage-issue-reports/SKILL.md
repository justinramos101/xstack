...
- Apply xstack's `principle-separate-before-serializing-shared-state` to source coordinates.
- Apply xstack's `principle-minimize-reader-load` and `unslop` skills to the final verdict.
...
3. Validate the normalized trigger with `node <copied Benny pack>/scripts/validate-trigger.mjs <configured-source-channel-id> < trigger.json`. Use its `SOURCE_CHANNEL_ID` and `SOURCE_THREAD_TS` output. A nonzero exit stops the run before any external write. The parser uses a nonempty `thread_ts`, otherwise `ts`.
...
Do a bounded source and history pass before choosing an owner or destination. Use xstack's `how` skill to trace the path from the reported action to the observed result. Use `why` when the report looks like a regression or touches defensive code.
