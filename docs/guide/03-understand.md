...
Editing code you don't understand is how subtle regressions ship. xstack gives you four ways in. `/how` explains what the code does now. `/why` digs up the reasons it's shaped that way. `/teach` blends both into one explanation. `/recall` rebuilds your own recent context on a topic.
...
Ask the question you have. [`/how`](../../skills/how/SKILL.md) reads the code and answers at the level of a senior engineer onboarding you onto the subsystem, with the runtime flow, the key types, and the non-obvious parts. For a big subsystem it fans out two to four read-only explorers first. For a narrow question it reads and explains.
...
You can chain the two. `do why first then how` is a good prompt when you suspect the history explains the mess.

## Get the whole picture with `/teach`

```text
/teach me how this PR changes retries. convince me it fixes the cause and not the symptom.
```

[`/teach`](../../skills/teach/SKILL.md) is for when a summary isn't enough. It runs `/how` and `/why`, or only one of them for a small change, and combines the findings into a plain explanation that builds up diagram by diagram. The "convince me" framing is worth copying. It turns the explanation into an argument you can challenge instead of a tour.
...
When another agent (or you, last week) left a branch unfinished:
