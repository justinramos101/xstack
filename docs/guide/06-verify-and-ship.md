...
Now the agent has three checks it can run. When the reply comes back, it should carry the exact commands and outputs. If a check couldn't run, a good reply says "inconclusive". Treat a confident reply without evidence as a warning.
...
For a small diff you don't fully trust, [`/blast-radius`](../../skills/blast-radius/SKILL.md) finds what it could break elsewhere. It names the one fact that makes the change safe, then proves that fact by running code instead of writing an essay about it.

## Create a project verification skill

The UI bullet above hides a real requirement. The agent needs a scripted way to drive your app. If your project already has one, skip this section. If not, run:
...
It writes `.agents/skills/verify-<app>/`. That directory holds agent-facing instructions with exact Launch, Doctor, Drive, Evidence, and Cleanup sections, plus a feature map under `features/`. The feature map indexes what the app does and what result proves each feature works. The skill ships a [worked feature-map example](../../skills/create-verification-skill/references/feature-map-example/) with a README index and one file per feature. Each feature file has the same four sections: sub-features, how to reach it, how to drive it, and gotchas. Before handing it over, the generator proves the skill once end to end: launch, doctor check, drive one feature, capture evidence, clean up. If that proof fails, don't use the output.
...
[`/maintain-verification-skill`](../../skills/maintain-verification-skill/SKILL.md) audits the generated skill: one read-only source reader per feature in parallel, then one live pass that drives every mapped feature. It ends in exactly one of three outcomes. `clean` means full coverage and nothing to ship. `changed` means one PR of proven corrections, confined to the verification skill's own directory. `blocked` names the blocker. It never edits product code. If the live pass catches a product regression, it reports the regression instead of editing the docs to match.
...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) works from a worktree, rebases the work into small ordered commits, cleans the diff, unslops the prose, and returns the PR link. Five narrow PRs beat one fat one, and stacked follow-ups beat a growing branch. A stack is a chain of PRs. The first PR targets the repository's trunk, whatever your default branch is called, and each later PR branches from the one below it and targets that branch. Reviewers see one small change at a time, and the changes land in order, bottom first.

The playbook talks to GitHub through the `gh` CLI. If you use [Origin](https://cursor.com/docs/origin), Cursor's git host, and its `origin` CLI is installed and can resolve the repository, the playbook uses that instead. Nothing in this page needs Graphite.
...
Green is not the same as safe. When you're ready to land a stack, or a single PR, say so:

```text
/poteto-mode land the stack.
```

The [Shipping playbook](../../skills/poteto-mode/playbooks/shipping.md) verifies each PR independently before it merges anything. One fresh agent per PR proves the behavior live, and the agent that judges a change is never the one that wrote it. Then Shipping lands only the contiguous verified run from the bottom, one PR at a time through whichever forge the PRs live on, and reports the first PR that breaks the chain. The lowest unmerged PR is called the frontier. Shipping keeps it green before it looks at anything above it. A verified PR above an unverified one waits, because merging it would pull the gap in underneath.
