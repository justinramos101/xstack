...
xstack ships 23 principles as individual skills. `/poteto-mode` reads their index at the start of every multi-step task, applies the ones the task triggers, and names each applied principle in its reply along with the decision it changed.
...
Say the agent is about to add a fourth adapter next to three existing ones:
...
Each phrase works because the rule behind it is specific. The agent still has to say, in its reply, which decision the rule changed. A principle named without the decision it changed is a sign the agent cited it without applying it.
...
- [Attack the Premise](../../skills/principle-attack-the-premise/SKILL.md) questions the assumption that two or more failed fixes shared, after measuring which parts of the system carry the problem.
...
- [Outcome-Oriented Execution](../../skills/principle-outcome-oriented-execution/SKILL.md) drives a rewrite to its target design instead of keeping every intermediate step stable with throwaway compatibility code.
...
The delegation principles govern parallel work:
