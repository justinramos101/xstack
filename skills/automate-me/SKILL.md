...
This skill orchestrates three others: an inline mining pass (see step 1), the host's skill-authoring workflow (authoring), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.

## Flow

### 0. Check for an existing skill

Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with `structured question tool` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. Use the exact active-session path or history API described in Host runtime. Don't glob across unrelated project transcript directories. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Mining misses intent that hasn't come up yet. Use the `structured question tool` tool (structured multi-choice) rather than asking the user to type from scratch.
...
Use the host's skill-authoring workflow skill to author the skill. Placement:
