...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the host's subagent tool. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (only when the host supports nesting; otherwise the root dispatches those workers). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use remote workers when available and isolated local worktrees otherwise. Local access is needed for the project's UI or CLI verification tool runtime verification (when installed). Reading local transcripts under the workspace-filtered transcript location described in Host runtime. Simulators and local IDE state. Auth that exists only here. Cloud agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget and available local or remote execution environment, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Use a remote checkout for expensive restacks when available; otherwise use an isolated local checkout and reduce concurrency to fit the machine.
...
- After a host restart, check each worker's actual status before resuming or replacing it. Remote work survives only if its service guarantees persistence; local work may have ended with the session. Re-read the standing orders and `units.tsv`, recompute the frontier, reconcile surviving work by PR, branch, and reported commit, restore needed coordinators from their stored briefs and current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
