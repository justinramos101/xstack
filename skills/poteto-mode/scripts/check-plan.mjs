...
const LANES = "Ten lanes using the configured swarm workers at the PR head";
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_MARKERS = ["Done condition.", "Skill source.", /30[- ]minute/, "status message"];
...
let raw;
try { raw = fs.readFileSync(file, "utf8").split(/\r?\n/); }
catch (error) { console.error(`${file}: ${error.message}`); process.exit(2); }
const problems = [];
...
for (let i = start; i < raw.length; i++) {
	...
	const prose = text
		...
		.replace(/!\[[^\]]*\]\([^)]*\)/g, "")
		.replace(/\]\([^)]*\)/g, "]");
	if (/<[^>\n]+>/.test(prose)) fail(n, "unfilled placeholder");
