...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript using the exact active-session path or history API described in Host runtime. Don't glob across unrelated project transcript directories. That reads unrelated private chats. Walk the log against what actually happened:
