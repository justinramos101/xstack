...
Before spawning investigators, enumerate the MCP tools available in this session through the host's tool catalog. Do not assume a filesystem directory exposes enabled integrations.
...
- Read-only investigation with the MCP read tools needed for this category. Use supported permission controls per Host runtime and prohibit writes in the brief.
...
- Read-only synthesis with MCP read access for citation checks, using the actual host controls per Host runtime.
